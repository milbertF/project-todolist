...
function App() {
  ...
  useEffect(() => {
    if (isOpen && inputRef.current) {
      // Optionally, you can use a very short delay (or none)
      setTimeout(() => {
  ...
  const handleSubmitTask = (e) => {
    ...
    if (editingTaskId) {
      ...
      createTask({ ...formData, id: Date.now() });
    }
    closeModal();
  };
          ...
          <dialog ref={dialogRef} id="dialogs" onClick={(e) => e.target === dialogRef.current && closeModal()}>
            <div className="modalCon">
              ...
              <form onSubmit={handleSubmitTask}>
                ...
                <button type="submit" aria-label={editingTaskId ? "Update Button" : "Add Button"}>{editingTaskId ? "Update" : "Add"}</button>
              </form>
            </div>
          </dialog>
        )}
          ...
          <div className="container">
            ...
            <div className="scdn-cvd">
              <div className="filterCon">
                {/* Priority Filter */}
                <div className="filters filterPriority" tabIndex={0} role="button" aria-label="Filter Priority">
                  <div className="leftf">
                    <span>Priority:</span>
                    <p>{priorityFilter}</p>
                  </div>
                  ...
                  <div className="filterPriorityBtnCon">
                    {priorityFilter !== "All" && (
                      <div className="filPri" onClick={() => setPriorityFilter("All")} tabIndex={0} role="button" aria-label="Filter for All Priority">
                        <p>All</p>
                      </div>
                    )}
                    {priorityFilter !== "High" && (
                      <div className="filPri" onClick={() => setPriorityFilter("High")} tabIndex={0} role="button" aria-label="Filter for High Priority">
                        <p>High</p>
                      </div>
                    )}
                    {priorityFilter !== "Medium" && (
                      <div className="filPri" onClick={() => setPriorityFilter("Medium")} tabIndex={0} role="button" aria-label="Filter for Medium Priority">
                        <p>Medium</p>
                      </div>
                    )}
                    {priorityFilter !== "Low" && (
                      <div className="filPri" onClick={() => setPriorityFilter("Low")} tabIndex={0} role="button" aria-label="Filter for Low Priority">
                        <p>Low</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="rdBtnCon">
                <button onClick={() => openModal()} aria-label='Add Task'><i className="fa-solid fa-plus"></i></button>
              </div>
            </div>
                              ...
                              {task.status === "Done" ? (
                                ...
                                <>
                                  <i
                                    ...
                                    onKeyDown={(e) => handleKeyDown(e, () => openModal(task))}
                                    tabIndex={0}
                                    role="button"
